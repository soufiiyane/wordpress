(function (elementor, $, window) {
  ...
  var TemplateLibraryTemplateView = Marionette.ItemView.extend({
    className: function className() {
      ...
      if ("hub_collections" === source) {
        classes +=
          " elementor-template-library-template-" + this.model.get("type");
      }
  ...
  var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
    ...
    addSourceData: function addSourceData() {
      ...
      this.$el.attr(
        ...
        isEmpty ? "empty" : hub_templates_lib.templates.getFilter("source")
      );
    },
    ...
    toggleFilterClass: function toggleFilterClass() {
      this.$el.toggleClass(
        ...
        !!(
          hub_templates_lib.templates.getFilter("text") ||
          hub_templates_lib.templates.getFilter("favorite")
        )
      );
    },
    onRender: function onRender() {
      if (
        "hub_collections" === hub_templates_lib.templates.getFilter("source")
      ) {
        this.setFiltersUI();
      }
    },
    onRenderCollection: function onRenderCollection() {
      ...
      if (
        "hub_collections" === hub_templates_lib.templates.getFilter("source") &&
        "page" !== hub_templates_lib.templates.getFilter("type")
      ) {
        this.setMasonrySkin();
      }
    },
    ...
    onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
      hub_templates_lib.templates.setFilter(
        "favorite",
    ...
    elementorModules.common.views.modal.Layout.extend({
      ...
      getTemplateActionButton: function (templateData) {
        ...
        if (
          ...
          viewId =
            "#tmpl-elementor-pro-template-library-activate-license-button-hub";
        }
      ...
      showPreviewView: function (templateModel) {
        this.modalContent.show(
          new TemplateLibraryPreviewView({
            url: templateModel.get("url").replace(/^http:\/\//i, "https://"),
          })
        );
        var headerView = this.getHeaderView();
        headerView.menuArea.reset();
  ...
  var Component = (function (ComponentModal) {
    ...
    Component.prototype.insertTemplate = function (args) {
      ...
      this.manager.requestTemplateContent(
        model.get("source"),
  ...
  var TemplateLibraryManager = function TemplateLibraryManager() {
    ...
    this.requestTemplateContent = function (source, id, ajaxOptions) {
      ...
      return elementorCommon.ajax.addRequest("hub_get_template_data", options);
    };
  ...
  elementor.on("document:loaded", function () {
    ...
    var $templateLibBtnStyle = $("<style />"),
      btnStyle = "";
    btnStyle +=
      ".elementor-add-section-area-button.hub-add-template-button { padding: 8px; vertical-align: bottom; }";
    ...
    $templateLibBtn.addClass(
      "elementor-add-section-area-button hub-add-template-button"
    );
    ...
    $previewContents
      .find("body")
      .delegate(".elementor-editor-element-add", "click", (ev) => {
        const timeout = setTimeout(() => {
          const $currentTarget = $(ev.currentTarget);
          const $parentSection = $currentTarget.closest(".elementor-element");
          const $addSectionSection = $parentSection.prev(
          ...
          $btnClone.on("click", () => {
            ...
            const modelIndex = elementor.elements.findIndex(
              (el) => el["cid"] === sectionModelId
            );
          ...
          $btnClone.insertAfter(
            $addSectionSection.find(
              ".elementor-add-section-area-button.elementor-add-template-button"
            )
          );
