...
class LD_Shortcode extends WPBakeryShortCode {
	...
	public function styles( ) {
		...
		if( 'off' !== $enabled_header ) {
			...
			if( isset( $header->post_content ) && has_shortcode( $header->post_content, $this->slug ) ) {
		...
		if( !is_404() && !is_search() && !is_archive() && isset( $header->post_content ) && has_shortcode( $post->post_content, $this->slug ) ) {
			foreach( (array)$this->styles as $handle ) {
				wp_enqueue_style( $handle );
			}
		}
		...
		if( 'off' !== $enabled_footer ) {
			...
			if( isset( $header->post_content ) && has_shortcode( $footer->post_content, $this->slug ) ) {
				foreach( (array)$this->styles as $handle ) {
					wp_enqueue_style( $handle );
				}
			}
