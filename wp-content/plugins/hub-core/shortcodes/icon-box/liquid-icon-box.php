...
class LD_Icon_Box extends LD_Shortcode {
	...
	public function get_params() {
		...
		$design_params = array(
			...
			array(
				'type'       => 'subheading',
				'param_name' => 'sh_label',
				'heading'    => esc_html__( 'Content Colors', 'landinghub-core' ),
			),
			array(
				'type'          => 'liquid_colorpicker',
				'only_solid'    => true,
				'param_name'    => 'text_color',
				'heading'       => esc_html__( 'Text Color', 'one-color' ),
			),
			array(
				'type'          => 'liquid_colorpicker',
				'only_solid'    => true,
	...
	protected function generate_css() {
		...
		if( !empty( $text_color ) ) {
			$elements[ liquid_implode( '%1$s p, %1$s .contents' ) ]['color'] = $text_color;
		}

		if( !empty( $hover_text_color ) ) {
			$elements[ liquid_implode( '%1$s:hover, %1$s:hover p, %1$s:hover .contents' ) ]['color'] = $hover_text_color;
		}
